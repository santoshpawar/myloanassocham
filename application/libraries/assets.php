...
class Assets {
	...
	function admin_icon_url($file,$level=""){
				$output = base_url()."$this->assets_folder/admin/images/icons/".$file;
				return $output;
			}
	
	function load_url($file,$level=""){
		if(!$level){
			$level = $this->default_level;
		}
		$type = strtolower(substr(strrchr($file, '.'), 1));
		...
		$filename=strtolower(substr($file,0,strripos($file,'.')));
		if(array_key_exists($type,$this->asset_types)){
			foreach($this->asset_types as $asset_type => $folder){
				if($type == $asset_type){
				 $output = site_url("$this->assets_folder/$folder/$level/").'/'.$filename;
			     return $output;
				break;
				}
			}

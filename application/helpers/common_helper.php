...
//echo $filename.'.'.$ext;die();
	...
	{//echo $utype_id; exit;
		...
		$opt["loan.msme_id"] = $users_details[0]->uid;
		$opt["msg_inbox.utype_id IN (3,4)"] = null;
		}else if($utype_id==2){
		$msme['ms.channel_partner_id']=	$CI->session->userdata('sesuserId');	
		$msme_list=$CI->admin_model->getMSMEArray($msme);
		//print_r($msme_list);exit;
		if(!empty($msme_list)){
		$other['list']="(loan.channel_partner_id=".$CI->session->userdata('sesuserId')." or loan.msme_id in(".$msme_list."))";
		}else{
		$opt["loan.channel_partner_id"] = $CI->session->userdata('sesuserId');	
		}	
		...
		else if($utype_id ==3 || $utype_id ==4)
		{
			...
			$opt["msg_inbox.post_to"] = $CI->session->userdata('sesuserId');
		}

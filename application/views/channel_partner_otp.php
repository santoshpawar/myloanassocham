...
<section id="inner-pages">
  <div class="container">
    ...
    <div class="row">
      <h2 class="module-head text-center">Channel Partner Registration</h2>
      <form name="form_otp" id="form_otp" method="post" action="<?php echo base_url() ?>home/check_otp_channel" style="line-height:35px;" class="div-margin">
		<?php
...
if($this->session->userdata('send_message')){ ?>
	...
	$this->session->set_userdata('send_message','');
		...
		<input type="hidden" name="advisor_mob_no" value="<?php echo $advisor_mob_no; ?>" >
          <div class="col-lg-2 col-xs-6"><button type="submit" class="btn btn-default">Submit</button></div>
        <div class="clear"></div>
        <div class="col-lg-12 top-margin20 resend bottom-margin60">Didn't receive OTP yet? <a href="<?php echo base_url();?>home/resend_otp_partner/<?php echo $this->session->userdata('sess_uid');?>">Resend OTP</a></div>
      </form>
    </div>
  </div>
</section>
